var ChromatographyRenderer = {
  ...
  drawLargeBeaker: function(ctx, g) {
    ...
    var bw = bk.w;
    var bh = bk.h;
    var isA3 = bk.type === 'glass';
    ctx.save();
    if (isA3) {
      var glassGrad = ctx.createLinearGradient(bx, by, bx + bw, by);
      glassGrad.addColorStop(0, 'rgba(180,210,240,0.22)');
      ...
      glassGrad.addColorStop(1, 'rgba(180,210,240,0.22)');
      ...
      ctx.beginPath();
      ctx.moveTo(bx + bw - 8, by);
      ctx.lineTo(bx + bw + spoutW2, by - 6);
      ctx.lineTo(bx + bw + spoutW2 - 4, by + 2);
      ctx.lineTo(bx + bw - 8, by + 4);
      ctx.strokeStyle = 'rgba(140,170,200,0.7)';
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.fillStyle = 'rgba(140,180,210,0.12)';
      ctx.fillRect(bx, by + bh - 3, bw, 4);
    }
    var solY = by + bh - bk.solventLevel * bh;
    ctx.fillStyle = isA3 ? 'rgba(150,200,240,0.3)' : 'rgba(170,210,245,0.35)';
    ctx.beginPath();
    ...
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = isA3 ? 'rgba(80,110,140,0.45)' : 'rgba(100,100,100,0.5)';
  ...
  drawInkBottles: function(ctx, g) {
    ...
    for (var i = 0; i < bottles.length; i++) {
      ...
      ctx.save();
      if (isA3) {
        var rgGrad = ctx.createLinearGradient(bx, by + 30, bx + b.w, by + 30);
        rgGrad.addColorStop(0, 'rgba(200,215,230,0.35)');
        rgGrad.addColorStop(0.2, 'rgba(230,242,255,0.15)');
        rgGrad.addColorStop(0.5, 'rgba(255,255,255,0.06)');
        rgGrad.addColorStop(0.8, 'rgba(230,242,255,0.15)');
        rgGrad.addColorStop(1, 'rgba(200,215,230,0.35)');
        ...
        ctx.fillRect(bx, bodyTop + 12, b.w, by + b.h - bodyTop - 12);
        ctx.globalAlpha = 1;
        ctx.restore();
        ...
        ctx.fillStyle = '#444';
        ctx.beginPath();
        ctx.ellipse(bx + b.w / 2, by + 2, neckW / 2 - 1, 2.5, 0, 0, Math.PI * 2);
        ctx.fill();
      } else {
      ...
      ctx.fillStyle = '#fff';
      var labelY = isA3 ? by + b.h - 26 : by + b.h - 22;
      ctx.fillRect(bx + 5, labelY, b.w - 10, isA3 ? 22 : 18);
      ctx.strokeStyle = '#ccc';
      ctx.lineWidth = 0.5;
      ctx.strokeRect(bx + 5, labelY, b.w - 10, isA3 ? 22 : 18);
      ctx.fillStyle = '#333';
      ctx.font = isA3 ? 'bold 7px sans-serif' : 'bold 7px sans-serif';
      ctx.textAlign = 'center';
      var lines = isA3 ? b.label.split('\n') : b.label.split(' ');
  ...
  drawLabels: function(ctx, g) {
    ...
    if (isA3) {
      ...
      ctx.fillText('Baseline', bk.cx - bk.w / 2 - 20, g.baseAbsY + 4);
      ctx.fillText('Cd²⁺', bk.cx + bk.w / 2 + 20, g.baseAbsY - 90);
      ctx.fillText('Pb²⁺', bk.cx + bk.w / 2 + 20, g.baseAbsY - 35);
      ctx.fillText('Solvent', bk.cx + bk.w / 2 + 20, bk.rimY + bk.h - 15);
      ctx.fillText('Wooden stick and clip', bk.cx, g.sim.stick.y - 20);
    } else {
      ctx.textAlign = 'center';
